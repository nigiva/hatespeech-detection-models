...
def load_bert_model(nb_labels):
    ...
    BERT for finetuning
    '''
    logger.info(f"transformers.BertTokenizer : bert-base-uncased")
    logger.info(f"transformers.AutoModel : bert-base-uncased")
    tokenizer = transformers.BertTokenizer.from_pretrained('bert-base-uncased')
    tr_model = transformers.AutoModel.from_pretrained('bert-base-uncased')
    model = TransformerClassifierStack(tr_model, nb_labels, freeze=True)
    return model, tokenizer


def load_bert_pretrained(path, nb_labels, lr=2e-5):
    '''
    Load BERT from checkout point (already trained on Hate Speech tasks)
    '''
    tokenizer = transformers.BertTokenizer.from_pretrained('bert-base-uncased')
    tr_model = transformers.AutoModel.from_pretrained('bert-base-uncased')
